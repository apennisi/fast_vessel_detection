...
{
    _img.copyTo(m_img);
    cv::cvtColor(m_img, m_gray, CV_BGR2GRAY);
                               ...
                               const int &_isr)
{
    ...
    /****BEGIN NEW PART*******/
    cv::Mat integralImg;
    cv::integral(m_gray, integralImg, CV_32FC1);
    integralImg = integralImg(cv::Rect(1,1, integralImg.cols - 1, integralImg.rows - 1));

    if(img_size.width != integralImg.size().width && img_size.height != integralImg.size().height)
    {
        utils::ImResample<float>::resample(integralImg, img_size, 1, integralImg.channels(), data);
    }
    else
    {
        data = integralImg.clone();
    }
    nTypes++;
